      ...
      <tr>
        ...
        <th style="text-align: center">작성자</th>
   		...
   		<% if(list.isEmpty()){ %>
  			...
  			<a onclick="location.href='<%= request.getContextPath() %>/careWriteBoardForm.bo'" class="btn btn-primary pull-right" style="background:#4895ef">임시보호 신청</a>
		...
		<div class="pageingArea" align="center">
			<!-- 맨 처음으로 -->
			<button onclick="location.href='<%= request.getContextPath()%>/boardCare.bo?currentPage=1'">&lt;&lt;</button>
			<!-- 이전 페이지로 -->
			<button onclick="location.href='<%= request.getContextPath()%>/boardCare.bo?currentPage=<%=pi.getCurrentPage()-1%>'" id="beforeBtn">&lt;</button>
			<script>
			...
			<% for(int p = pi.getStartPage(); p <= pi.getEndPage(); p++){ %>
			 	<% if(p == pi.getCurrentPage()){ %>
			 		...
			 		<%}else{ %>
				<button id="numBtn" onclick="location.href='<%=request.getContextPath() %>/boardCare.bo?currentPage=<%=p %>'"><%=p %></button>
				<% } %>
			<% } %>
			<!-- 다음 페이지로 -->
			<button onclick="location.href='<%= request.getContextPath()%>/boardCare.bo?currentPage=<%=pi.getCurrentPage()+1%>'" id="aftereBtn">&gt;</button>
			<script>
			...
			<!-- 맨 마지막으로 -->
		    <button onclick="location.href='<%= request.getContextPath()%>/boardCare.bo?currentPage=<%=pi.getMaxPage()%>'">&gt;&gt;</button>
		</div>
